...
|---|------|-----------------|------------|------------|
| 1 | Fetch | tRPC exclusivo. Server Components usan createServerCaller(). Client Components rehidratan — no fetch propio. REST solo para bootstrap de cookies (ADR-004). | Ningún fetch manual sin justificación documentada. | ✅ sass-front + dashboard-front completos · ⚠️ ecommerce-front: lib/store/client.ts fetch REST pendiente de migrar a tRPC server caller |
