#!/usr/bin/env bash
echo "=== Fix: tsconfig.base.json + prisma postinstall ==="

node - << 'JSEOF'
const fs = require('fs');

// ── FRONTENDS: agregar COPY tsconfig.base.json ────────────────────────────────
const frontends = [
  {
...
// ── BACKENDS: --ignore-scripts para evitar postinstall de prisma ──────────────
// Luego correr prisma generate manualmente después de copiar el schema
